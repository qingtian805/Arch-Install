...
fcitx() {
    ...
    echo -e "# Setting up fcitx5 environments\nexport XMODIFIERS=@im=fcitx" | sudo tee /etc/profile.d/fcitx.sh
    
    if [[ "$*" =~ "xorg" ]]; then 
        echo -e "${fcitx_env_full}" | sudo tee -a /etc/profile.d/fcitx.sh
    fi
}